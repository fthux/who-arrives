...
export interface Enrichment {
  ...
  countryOrRegion?: Partial<CountryOrRegion> & { flag?: string | null };
  continent?: { name: string | null };
  location?: { time: LocalTime | null };
}

const countryData: Readonly<Record<string, CountryOrRegion>> = countries;
  ...
  for (const field of fields) {
    switch (field) {
      ...
      case 'capitals': case 'currencies': case 'languages': case 'callingCodes':
        ...
        Object.assign(result.countryOrRegion ??= {}, { [field]: structuredClone(country?.[field] ?? null) });
        break;
    }
  }
  return result;
